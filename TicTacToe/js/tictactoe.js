...
function placeXOrO(squareNumber) {
    //Thid condition ensures a square hasn't be selected already.
    //The .some() method is used to checj each element of selectedSquare array to
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        checkWinConditions();
        //THis condition is for changing the active plaer.
        if (activePlayer === 'X') {
        ...
        //Returning true is needed for our comutersTurn() function to work.
        return true;
    }
...
//It is used to check for eah win condition.
function arrayIncludes(squareA, squareB, squareC) {
    //The nrxt 3 variables will be used to check for 3 in a row.
    const a = selectedSquares.includes(squareA);
    const b = selectedSquares.includes(squareB);
    const c = selectedSquares.includes(squareC);
    // If the 3 variables we pass are all included in our array true is
    //returned and our else if condition execur=tes the drawWinLine function.
...
function animateLineDrawing() {
    ...
    if (x1 <= x2 && y1 <= y2) {
        ...
        if (x < x2) { x += 10; }
        //This conditition adds 10 to the previous end y point.
        if (x < y2) { y += 10; }
        //This condition cancels our aniation loop if reach the end points.
